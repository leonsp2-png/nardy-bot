...
async def import_data(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text(
            ...
            "Пример: /import сергей-коля 20-10",
            reply_markup=get_main_keyboard(),
        )
        return
    ...
    try:
        ...
        session.commit()
        response = f"✅ Импортировано: {imported}"
        if errors:
            response += "\n" + "\n".join(errors)
        ...
        session.rollback()
        logger.exception("Ошибка при импорте")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info("MSG: %s, state: %s", text, context.user_data.get("state", "menu"))
...
async def add_player(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if not name:
            await update.message.reply_text("❌ Имя не может быть пустым.", reply_markup=get_main_keyboard())
        elif session.query(Player).filter_by(username=name).first():
...
async def start_game(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.info("Отправлены кнопки выбора, msg_id=%s, state=choose_p1", msg.message_id)
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info("CALLBACK: data=%s, state=%s", data, state)
    ...
    if state == "choose_p2" and data.startswith("p2_"):
        ...
        session = Session()
        try:
            p1 = session.get(Player, context.user_data["p1"])
            p2 = session.get(Player, p2_id)
    ...
    if state == "choose_result" and data in ["win1", "mars1", "win2", "mars2"]:
        ...
        try:
            p1 = session.get(Player, context.user_data["p1"])
            p2 = session.get(Player, context.user_data["p2"])
    ...
    if state == "confirm" and data == "save":
        ...
        try:
            ...
            winner = session.get(Player, win) if win else None
            mars_text = " (МАРС!)" if mars else ""
            ...
            logger.exception("Ошибка при сохранении игры")
    ...
    logger.warning("Необработанный callback: %s, state=%s", data, state)
        ...
        for p1 in players:
            for p2 in players:
                ...
                games = (
                    ...
                    .filter(
                        or_(
                            and_(Game.player1_id == p1.id, Game.player2_id == p2.id),
                            and_(Game.player1_id == p2.id, Game.player2_id == p1.id),
                        )
                    )
                    .all()
                )
...
async def show_last_games(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for g in games:
            p1 = session.get(Player, g.player1_id)
            p2 = session.get(Player, g.player2_id)
            if not p1 or not p2:
                continue
